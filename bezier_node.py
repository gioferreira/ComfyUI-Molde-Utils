...
class BezierMapping:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "t_value": ("FLOAT", { "default": 0.0, "min": 0.0, "max": 1.0, "step": 0.01 }),  # Input t
    ...
    def compute(self, t_value, P0, P1, P2, P3, output_min, output_max):
        ...
        y_points = [output_min + (output_max - output_min) * bezier(t, P0, P1, P2, P3) for t in t_values]

        # Create a new figure and plot
        fig, ax = plt.subplots(figsize=(4, 4))
        ax.plot(t_values, y_points, color='black', linewidth=1.5)
        ax.scatter([t_value], [mapped_y], color='red', zorder=5)  # Highlight the input/output point
        ...
        img_tensor = img_tensor.permute([0, 2, 3, 1])  # Adjust to match your expected format
